...
## 2026-04-17 — v1.0 ships PH-BC-001 relative-mode as rank-reliable but threshold-unreliable on homogeneous-Dirichlet

**Context.** Week 4 Task 4 Step 0 generated three real-ML CI dumps (`unet_regressor`, `fno`, `ddpm`) from `tyy0811/laplace-uq-bench` checkpoints at `sample_index=0` of the published test set. The interpretation pass after extraction produced a per-model verdict table for `physics-lint check` on each dump; the pre-scope expectation was `unet=PASS, fno=FAIL (PH-BC-001 hero), ddpm=PASS`. Actual result: all three exit 1, failing `PH-BC-001`, `PH-BC-002`, and `PH-RES-001` — with `PH-POS-002` the only rule that cleanly discriminated (FNO FAIL raw=0.078, UNet/DDPM PASS raw=0).

**Root cause.** `PH-BC-001` in relative mode divides the raw boundary error by `avg|boundary_target|`, with a denominator floor at machine epsilon (~2.2e-16) when underflow would otherwise happen. `sample_index=0` of the laplace-uq-bench test set is a homogeneous-Dirichlet problem (boundary target ≡ 0) — the floor dominates. Back-solving raw / ratio gives denominator = 2.22e-16 (exact float32 epsilon) for all three models, producing ratios of 7.04e13 (UNet) / 8.64e14 (FNO) / 4.70e13 (DDPM) that mechanically trip the FAIL threshold for any non-zero raw error. The `ratio` comparison is rank-ordering reliable (FNO > UNet > DDPM), but the absolute `FAIL` verdict is a threshold artifact, not a physics signal. `PH-RES-001` has the same pattern with a 1e-12 floor.

**V1 decision.** Ship v1.0 with the documented limitation rather than block on a fix. Rationale:

1. Rank-ordering is still correct on both rules — the Week 2½ 3-axis dogfood ranking agreement is unaffected. The v1.0 dogfood claim ("DDPM best, FNO worst on `bc_err`") survives.
2. `PH-POS-002` provides a clean absolute-threshold hero that matches the "unique physics discrimination" intent of the planned `PH-BC-001` hero better than the broken-denominator version does. Hero pivoted accordingly — see Week 4 plan framing commit amendment.
3. The honest framing ("all three trained surrogates failed at least one physics check; FNO is most severely flagged") is stronger marketing than "physics-lint caught the one we pointed it at." Disclosure reads as product honesty, not as a retracted claim.
4. The v1.1 fix is ~1 day of calibration work (see `docs/backlog/v1.2.md` 2026-04-17 denominator regularization entry); holding v1.0 for it would be scope creep against the time-boxed release.

The v1.0 README gains a "known limitations" section explicitly disclosing the rank-reliable-but-threshold-unreliable property on `PH-BC-001` and `PH-RES-001` for homogeneous-Dirichlet samples, with a v1.1 pointer. Users running CI with `continue-on-error: false` are advised to prefer absolute mode (available via per-rule override in v1.1) or to gate on rules other than these two until the floor is regularized.

**Generalized lesson.** Relative-mode thresholds that divide raw error by a norm of the target are degenerate when the target is identically zero, even with a machine-epsilon floor. Machine epsilon is not a physical floor — it's a numerical-underflow guard. A rule that passes a synthetic test case with `target ≠ 0` can still produce meaningless verdicts on a real-world sample where `target = 0` by design (homogeneous BCs, reference field at zero, etc.). Future rules with relative-mode thresholds need a *calibrated* denominator floor in `floors.toml`, not a machine-epsilon guard. This is discoverable at calibration time if a homogeneous-target sample is included in the calibration mix; the v1.0 calibration run did not include one.

**Recovery cost note.** The interpretation pass (~45 min) caught the mismatch between designed and actual hero before the PR opened, preventing a Security-tab screenshot captured under the wrong framing. This is the value of the tripwire-pause pattern from the 2026-04-17 Phase 3 lesson, applied in the correct direction: expected outcome did not match the design, paused to interpret, pivoted the hero under the lesson that landed. Had the pivot happened after push (screenshot captured against PH-BC-001, README written against it), the unwind cost would have been the screenshot recapture + README rewrite + a public amendment commit; pre-push the cost was just the plan/README/backlog edits in a single commit.
