...
A `SKIP` is a principled-dispatch outcome, not a gap: the substrate-detection layer fired and declined to evaluate a rule whose assumptions don't hold on that substrate. An `N/A` is substrate- or scope-incompatibility. A *deferred* cell names the amendment that will close it. The LagrangeBench equivariance rows are harness-methodology measurements on the particle substrate (see Case study 01 above); the MeshGraphNet conservation rows are shipped-rule runs on a materialized `GridField`.
