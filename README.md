...
The dogfood suite checks physics-lint against trained Laplace surrogates; the external-validation suite checks it against classical PDE theory. The **rollout-anchor** track checks the third leg: whether the same SARIF rule schemas run *unmodified* across architecturally- and substrate-distinct third-party neural-physics stacks — take a trained, published model, run its rollout, lint the output through the public `physics-lint` API (with a thin documented harness where the public Field/rule API doesn't natively accept the data, e.g. particle clouds).

**Case study 01 — LagrangeBench (TUM, NeurIPS 2023): SEGNN + GNS.** Three rounds exercise the schema-uniformity claim from different angles. The `PH-CON` conservation schema runs across SEGNN-TGV2D and GNS-TGV2D rollouts and emits byte-identical rows on both, with a dissipative-system skip-with-reason gate exercised end-to-end against real upstream output. The `PH-SYM` equivariance schema runs across the same two stacks at single-step inference and correctly distinguishes them — SEGNN's E(2)-equivariance is exact-by-construction (error ~2.3e-7, at the float32 noise floor); GNS's is approximate-by-training (bimodal ~3.6e-4 to ~3.5e-2). The substrate-detection layer extends to a second LagrangeBench substrate class (open-driven-dissipative) via dam-break-2D, where both stacks (N = 12 trajectories) emit byte-identical structural rows under the substrate's verdict. Together the three rounds bilaterally exercise the claim: same machinery, different values when stack architecture matters; same machinery, identical values when substrate determines the verdict.

**Case study 02 — NVIDIA PhysicsNeMo MeshGraphNet (in progress).** A mesh-domain integration: a published NGC MeshGraphNet checkpoint (cylinder-wake vortex shedding), linted per-timestep through the public Field/rule API. Phase 1's pre-rollout audit passes — NGC v0.1.0-format weights load into the v2.0.0 model via a documented name-remap + edge-MLP column-reorder adapter, and the model reproduces NGC's shipped sample to RMSE ≈ 3.2e-3 (≈ 1.36× the Pfaff et al. reference), clearing the pre-registered reproduction gate (so the planned FNO-on-Darcy substitute case study is *not* triggered). Case study 02 is framed as a *falsification surface* for the schema-uniformity claim (does it hold beyond LagrangeBench?), not an already-completed validation.

Full methodology trail (per-round writeups, decision log, case study 02 design + Phase 1 plan) in [`external_validation/_rollout_anchors/methodology/`](external_validation/_rollout_anchors/methodology/).
