...
|---|---|---|---|
| `PH-SYM-001/002` equivariance | 80/80 PASS, ~2.3e-7 — exact-by-construction | 0/80 PASS, bimodal ~3.6e-4–3.5e-2 — approximate-by-training | N/A — `PH-SYM-*` is particle-side only by scope |
| `PH-SYM-003` SO(2) | SKIP — PBC-square breaks SO(2) | SKIP — same reason | N/A — particle-side only |
...
A `SKIP` is a principled-dispatch outcome, not a gap: the substrate-detection layer fired and declined to evaluate a rule whose assumptions don't hold on that substrate. An `N/A` is substrate- or scope-incompatibility. A *deferred* cell names the amendment that will close it.
