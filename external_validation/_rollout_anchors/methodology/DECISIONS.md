...
**Stat-fit parameters and the open question they leave.** `compute_cylinder_flow_stats` fits the two stats files via `VortexSheddingDataset(split="train", num_samples=1000, num_steps=600, noise_std=0.02)` — the dataset class's own `__init__` defaults, i.e. the **full** DeepMind cylinder_flow train split (1000 trajectories × all 600 time steps) with DeepMind/modulus-standard input-noise injection. Noise is added to the train-split velocities *before* stats are computed (`vortex_shedding_dataset.py:127-133` @ `1ca85d65`), so `velocity_std` and especially `velocity_diff_std` carry a `~noise_std**2` term — this is what the checkpoint was trained to expect, so it is intentional. `torch.manual_seed(0)` makes the noise-injected fit reproducible. The example `examples/cfd/vortex_shedding_mgn/conf/config.yaml` uses `(num_training_samples=400, num_training_time_steps=300)` for faster training; the modulus-era 2023 config the NGC checkpoint was actually trained against is **not documented upstream**, so the full-train default is chosen as the most reproducible anchor (no arbitrary subsample, no time-axis truncation). **Whether these stats match the checkpoint's normalization is empirically tested by Task 7's NGC sample reproduction** (verdict 4; 10⁻³ tolerance) — if Task 7 fails, the stat-fit parameters are the first suspect and a `(400, 300)` re-fit is the cheap retry before invoking the FNO-on-Darcy fallback (design §3.1.A).
