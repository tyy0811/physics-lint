...
| `lint_npz_dir.py` | `lint_npz_dir(empty_dir)` | Raise `EmptyNpzDirectory` (no silent empty SARIF) |
| `emit_sarif.py` driver | Both stack directories present in local mirror | Raise `MissingLocalMirror` with the `modal volume get` command in the message |
