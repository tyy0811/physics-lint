...
>
> **Plan-vs-reality drift #5 — externally-induced (master-side, NOT branch-side, added at PR #11 merge integration, 2026-05-15).** Drifts #1-4 above are branch-side (this branch's execution diverged from this plan's templates and the corrections live in the committed artifacts). Drift #5 is structurally different: PR #10 (round-code-1) merged to master ahead of this work and re-emitted the LB SARIFs at new shas (`8e49339469` → `96ce8ed0eb` for the conservation SARIFs; `255af5de8d` → `aeb4a53e87` for the eps SARIFs). This Phase 3 plan documents the LB shas at write-time (the `*_tgv2d_8e49339469.sarif` glob in Tasks 2 + 3 + later examples reflects branch-creation-time state); the published cross-stack table at merge-time reflects the post-merge state. Mechanism: parallel-branch SARIF re-emission discovered at merge integration. Resolution at PR #11 merge: renderer test glob updated to `*_tgv2d_96ce8ed0eb.sarif`, golden fixture regenerated against the post-PR-#10 LB SARIFs (row content byte-identical modulo the sha annotations — verified by content-equivalence diff before regeneration), published cross-stack-conservation-table artifact's provenance section + Rederivability example carry the new shas with a "Provenance footnote" naming the publication-time-vs-write-time distinction. The plan's `8e49339469` references stay as-is (historical snapshot; this drift entry is the merge-time pointer to the publication-time state).
