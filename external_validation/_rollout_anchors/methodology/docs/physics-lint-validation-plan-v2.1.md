...
**Date:** 2026-05-10 (initial) / 2026-05-11 (round-3 + round-prose-1 + round-codex-2 + round-codex-3 absorptions appended)
...
### v2.1 round-codex-3 absorption — 2026-05-11

§9 fold-in round-codex-3 (Codex adversarial review of the rung-4c PR #9 at sha `7ac5be1` — i.e., the post-round-codex-2 state; target diff vs `409bee0`). Two HIGH findings:

- **Finding 1 (HIGH; cell 2): Manifest classification not bound to rollout directory.** Pre-fix, `_REQUIRED_CLASSIFICATION_FIELDS = ("inference_returncode", "aborted_at_step")` — `rollout_subdir` was persisted but never validated. An operator with a timed-out `segnn_dam2d` rollout could copy the clean manifest from `gns_dam2d/` into `segnn_dam2d/`, and the classifier would return `from_completed_inference` — bypassing the round-codex-2 missing-required + the round-3 invalid-manifest gates simultaneously. **Absorbed** by: (a) adding `rollout_subdir` to `_REQUIRED_CLASSIFICATION_FIELDS`; (b) adding a basename-comparison check (`Path(persisted["rollout_subdir"]).name == Path(classified_path).name`) — mismatch → `STATUS_MANIFEST_INVALID`; (c) updating `persist_inference_manifest_to_rollout_subdir` to auto-fill `rollout_subdir` from the destination path if not in the manifest dict (safety net for minimal-manifest callers). 7 new tests covering basename-mismatch attack, missing-field, both classification paths (completed + aborted) with matching basenames, trailing-slash normalization, auto-fill, and no-overwrite-of-explicit.
- **Finding 2 (HIGH; cell 2): Renderer accepts duplicate stack labels.** Pre-fix, the renderer collected `(model_name, dataset_name)` labels via simple append + indexed cells by `(rule_id, stack_label)`. If two SARIFs for the same stack landed in the same directory (e.g., post-re-emission with old SARIFs not cleaned up; `--include-glob "*dam2d*.sarif"` picks up both), the rendered table would have duplicate columns whose cells were overwritten under one key — silent corruption rather than fail-loud. **Absorbed** by: (a) adding `DuplicateStackLabelError` exception class; (b) adding pre-cell-building duplicate-label detection that raises with both file paths + remediation guidance (clean up stale SARIFs OR use a sha-specific glob OR pass exact file paths). 2 new tests covering duplicate-detection-raises + distinct-labels-still-render.

**Pattern C self-application:** both findings are **cell 2 (novel-in-scope)** — neither was a re-discovery of prior decisions, both fit the v2.1 + rung-4c-branch scope. The round-codex-3 pass is the *second consecutive* cross-review round to surface HIGH findings on the same gate / renderer surfaces previously absorbed (rounds 1-3, round-codex-2 already touched them). This is methodologically interesting: the gate's *correctness contract* has been refined in stages — each round absorbed a real fail-open, but each round's fix opened a different attack surface visible only at the next layer of adversarial review. This is evidence the cross-review-gate discipline (smoke + source + cross per §1.4) is doing real work; sub-discipline observation: **fail-open paths in safety-critical code tend to surface in layered fashion** — fixing one reveals adjacent ones. Worth flagging for case study 02's safety surfaces.

**Pattern B self-application:** finding 1's fix is the *second* clean pattern-B instance post-round-prose-1 reformulation. Single artifact (the classifier), multi-instance (manifests legitimately co-existing across multiple rollout subdirs in the same Volume), hidden assumption ("classification fields are sufficient to identify the run; rollout_subdir is informational"), response (generalize the validation to include directory binding, not special-case copied manifests). Finding 2's fix is **not** pattern-B per §1.2's necessary conditions — the renderer wasn't hiding a single-use-case assumption; it was missing a defensive check for a workflow that always could have produced duplicates. Closer to "defensive validation gap" than "implementation-time hidden assumption." This is useful negative evidence for §1.2: not every safety fix is pattern B, even when it shares some surface features.

**On testing.** 9 new tests landed: 7 on classifier/persist (binding to rollout dir) + 2 on renderer (duplicate-label detection). Full suite: 822 passed, 1 skipped (813 baseline from round-codex-2 + 9 new). No regressions.
